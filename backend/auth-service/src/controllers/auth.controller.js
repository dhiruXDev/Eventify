...
const { sendPasswordResetEmail, sendAccountCreationEmail } = require('../utils/email.utils');
...
exports.register = async (req, res, next) => {
  try {
    ...
    const user = await User.create({
      ...
      password,
      college,
      role: role || 'participant' // Use provided role or default to 'participant'
    });
    ...
    // Send account creation email (don't wait for it to complete)
...
exports.updateProfile = async (req, res, next) => {
  try {
    const { firstName, lastName, college, photo } = req.body;
    ...
    if (photo) user.photo = photo; // Update profile photo
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    const resetUrl = `https://eventify.vercel.app/reset-password/${resetToken}`;
    ...
    try {
      ...
      // Check if email was sent in fallback mode (logged but not actually sent)
      const isFallbackMode = emailResult.messageId === 'fallback-mock-id';
      
      if (isFallbackMode) {
        console.log('⚠️  Email sent in fallback mode - check server logs for reset URL');
        console.log('⚠️  Reset URL for testing:', frontendResetUrl);
      } else {
      ...
      const responseData = {
        ...
        message: isFallbackMode 
          ? 'Password reset link generated. Check server logs for the reset URL (email service not configured).'
          : 'Password reset email sent'
      };

      // Only include reset URL in development mode for testing
      if (process.env.NODE_ENV === 'development' && isFallbackMode) {
        responseData.resetUrl = frontendResetUrl;
        responseData.note = 'Email service not configured - using fallback mode. Use the resetUrl above for testing.';
      }
...
/**
 * @desc    Get user by ID (for service-to-service calls)
