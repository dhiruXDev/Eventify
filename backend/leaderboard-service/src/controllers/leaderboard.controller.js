...
exports.updateParticipantScore = async (req, res, next) => {
  try {
    const { eventId, userId } = req.params;
    const { points, achievements, club, userPhoto } = req.body;
    
    // Fetch user photo from User collection if not provided
    let photoToStore = userPhoto || '';
    if (!photoToStore) {
      try {
        // Validate userId is a valid ObjectId
        if (mongoose.Types.ObjectId.isValid(userId)) {
          const userInfo = await mongoose.connection.db.collection('users').findOne(
            { _id: new mongoose.Types.ObjectId(userId) },
            { projection: { photo: 1 } }
          );
          if (userInfo && userInfo.photo) {
            photoToStore = userInfo.photo;
          }
        }
      } catch (err) {
    ...
    if (!scoreEntry) {
      ...
      if (existingUserEntry) {
        if (existingUserEntry.college) {
        college = existingUserEntry.college;
        }
        if (existingUserEntry.club) {
          clubName = existingUserEntry.club;
        }
      ...
      if (club !== undefined && club !== null) {
        scoreEntry.club = club;
      }
      
      // Update user photo if provided (always update if a new photo is provided)
      if (photoToStore) {
        scoreEntry.userPhoto = photoToStore;
      }
...
exports.getTopPerformers = async (req, res, next) => {
  try {
    ...
    const topPerformers = await Leaderboard.aggregate([
      ...
      // Limit to top 20
      { $limit: 20 },
      // Lookup user photo from User collection
      {
        $lookup: {
          from: 'users',
          localField: '_id',
      ...
      // Project fields for output
      {
        $project: {
          ...
          userPhoto: { 
            $cond: {
              if: { $and: [{ $ne: ['$userPhoto', ''] }, { $ne: ['$userPhoto', null] }] },
              then: '$userPhoto',
              else: { $ifNull: ['$userInfo.photo', ''] }
            }
          },
